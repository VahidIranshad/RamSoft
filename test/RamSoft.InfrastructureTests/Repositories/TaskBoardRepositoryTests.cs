...
{
    [TestFixture]
    public class TaskBoardRepositoryTests
    {
        ...
        public async Task GetAll_Between_Repository_And_Context()
        {
            ...
            {
                var repository = new TaskBoardRepository(context);

                var list = await repository.GetAll(CancellationToken.None);
                var dbList = await context.TaskBoardDbSet.ToListAsync(CancellationToken.None);
        ...
        public async Task Get_By_Id_Between_Repository_And_Context(int id)
        {
            using (var context = Fixture.CreateSQLContext())
            {
                ...
                var dbItem = await context.TaskBoardDbSet.AsNoTracking().Where(p => p.Id == id).FirstOrDefaultAsync(CancellationToken.None);
        ...
        public async Task Get_By_Expression_Between_Repository_And_Context()
        {
            using (var context = Fixture.CreateSQLContext())
            {
                ...
                Expression<Func<TaskBoard, bool>> expression = (p => p.Id > 0);

                var list = await repository.Get(expression, CancellationToken.None);
                var dbList = await context.TaskBoardDbSet.Where(expression).ToListAsync(CancellationToken.None);
        ...
        public async Task Get_By_Ids_Between_Repository_And_Context()
        {
            using (var context = Fixture.CreateSQLContext())
            {
                ...
                var dbList = await context.TaskBoardDbSet.Where(p => ids.Contains(p.Id)).ToListAsync(CancellationToken.None);
        ...
        public async Task Exists_Between_Repository_And_Context(int id)
        {
            using (var context = Fixture.CreateSQLContext())
            {
                ...
                var dbItem = await context.TaskBoardDbSet.AsNoTracking().Where(p => p.Id == id).AnyAsync(CancellationToken.None);
        ...
        public async Task Add_Item()
        {
            using (var context = Fixture.CreateSQLContext())
            {
                ...
                var newItem = new TaskBoard() { Name = "test" };

                var item = await repository.Add(newItem, CancellationToken.None);
                var dbItem = await context.TaskBoardDbSet.Where(p => p.Id == item.Id).FirstOrDefaultAsync(CancellationToken.None);
        ...
        public async Task Update_Item()
        {
            using (var context = Fixture.CreateSQLContext())
            {
                ...
                var updateItem = new TaskBoard() { Id = 1, Name = "test", DefaultStatesId = 1 };

                await repository.Update(updateItem, CancellationToken.None);
                var dbItem = await context.TaskBoardDbSet.Where(p => p.Id == updateItem.Id).FirstOrDefaultAsync(CancellationToken.None);
        ...
        public async Task Delete_Item()
        {
            using (var context = Fixture.CreateSQLContext())
            {
                ...
                var dbItem = await context.TaskBoardDbSet.Where(p => p.Id == 1).FirstOrDefaultAsync(CancellationToken.None);
